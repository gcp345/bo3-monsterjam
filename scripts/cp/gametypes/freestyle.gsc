...
function main()
{
	...
	level.monster_jam_trucks_use_freestyle = true;
	level.monster_jam_use_commentator = true;
	level.monster_jam_trucks_enable_first_time_bonus = true;
	...
	setscoreboardcolumns( "score", "pointstowin", "destructions", "", "" );
}
...
//	level flag::wait_till( "checkpoints_ready" );
	...
	{
		player thread MonsterJamComm::DoCommentation( "introfreestyle" + player.truck.truck_type );
	}
...
{
	pixbeginevent("freestyle:onSpawnPlayer");
...
// use our predefined 'freestyle' spawns
function custom_monster_truck_spawn_callback()
{
	str_targetname = MonsterJamUtility::GetRaceString() + "_spawn_" + self GetEntityNumber();
...
function custom_monster_truck_respawn_callback()
{
	...
	best_respawn = array::random( a_respawns );

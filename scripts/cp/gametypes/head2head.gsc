...
function main()
{
	...
	level.monster_jam_head2head_mode = true;
...
//	spawning::create_map_placed_influencers();
	...
	level thread setupDirectionPointers();
...
function setupCheckPoints( str_targetname )
{
	...
	for( i = 0; i < 2; i++ )
	{
		...
		DEFAULT( e_start_line.dir_index, [] );
		e_start_line.dir_index[ i ] = 0;

		next_index = 1;

		prev_checkpoint = e_start_line;
		ARRAY_ADD( level.checkpoints[ i ], prev_checkpoint );

		while( !b_full_lap )
		{
			// anything odd goes the opposite direction
			if( i == 1 )
				e_next_checkpoint = GetEnt( prev_checkpoint.targetname, "target" );
			else
			...
			if( IsDefined( e_next_checkpoint ) )
			{
				...
				ARRAY_ADD( level.checkpoints[ i ], e_next_checkpoint );

				DEFAULT( e_next_checkpoint.dir_index, [] );
				e_next_checkpoint.dir_index[ i ] = next_index;
...
function checkPointThink()
{
	...
	while( true )
	{
		...
		if( IsDefined( e_who ) )
		{
			...
			n_direction = player GetDirection();

			// grab the indexes for easy checks down below
			b_start_line = IsDefined( self.script_noteworthy ) && self.script_noteworthy === "start";
			this_index = self.dir_index[ n_direction ];

			n_next = this_index + 1;
			// check if the next checkpoint is the finish line
			if( n_next == level.checkpoints[ n_direction ].size )
			{
				n_next = 0;
			}
			next_index = level.checkpoints[ n_direction ][ n_next ].dir_index[ n_direction ];

			n_prev = this_index - 1;
			// check if this is the first checkpoint
			if( n_prev < 0 )
			{
				n_prev = level.checkpoints[ n_direction ].size - 1;
			}
			prev_index = level.checkpoints[ n_direction ][ n_prev ].dir_index[ n_direction ];
			...
			if( b_start_line )
			{
				...
				else if( player.checkpoints.size == level.checkpoints[ n_direction ].size )
				{
					player.checkpoints = [];
					player.cur_checkpoint = 0;
...
function position_distance_tracker()
{
	...
	while( IsDefined( self ) )
	{
		...
		{
			dist = Distance( self.origin, level.checkpoints[ n_direction ][ 0 ].origin );
		...
		// check if the next checkpoint is the finish line
		if( next_idx == level.checkpoints[ n_direction ].size )
		{
			next_idx = 0;
		}
		next_chk = level.checkpoints[ n_direction ][ next_idx ];

		curr_idx = self.cur_checkpoint;
		// check if this is the first checkpoint
		if( curr_idx < 0 )
		{
			curr_idx = level.checkpoints[ n_direction ].size - 1;
		}
		curr_chk = level.checkpoints[ n_direction ][ curr_idx ];
		...
		track_completion = Min( MapFloat( 0, chk_dist, ( next_idx == 0 ? 1 : ( next_idx / ( level.checkpoints[ n_direction ].size ) ) ), ( curr_idx / ( level.checkpoints[ n_direction ].size ) ), player_dist ), 0.9999 );
...
function SortUnfinishedPlayers()
{
	...
	foreach( player in level.players )
	{
		...
		player.temp_time = ( player MonsterJamUtility::CalculateFinishTime( player GetDirection() ) - level.start_time ); // 0 is the default index for time calculations
...
function custom_monster_truck_respawn_callback()
{
	...
	// we need to pick an actual respawn point.
	best_respawn = undefined;
	best_score = ( 100000 * 100000 );

	for( i = 0; i < a_respawns.size; i++ )
	{
		...
		if( !IsDefined( respawn.script_int ) || respawn.script_int !== n_direction )
		{
			continue;
		}
...
{
	if( !IsDefined( self.headtohead_direction ) )
	{
		self.direction_index = self GetEntityNumber() % 2;
	}
	return self.direction_index;
}

function setupDirectionPointers()
{
	level flag::wait_till( "all_players_spawned" );

	a_pointers = GetEntArray( "direction_pointers", "targetname" );

	if( a_pointers.size == 0 )
	{
		return;
	}

	foreach( pointer in a_pointers )
	{
		pointer SetInvisibleToAll();
	}
	...
	foreach( player in level.players )
	{
		...
		foreach( pointer in a_pointers )
		{
			if( n_direction == pointer.script_int )
			{
				pointer SetVisibleToPlayer( player );
			}
		}
	}
}
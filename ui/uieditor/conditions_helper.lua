...
function IsCACItemLockedHelper( self, element, controller )
	local f10_local0 = CoD.perController[controller].classModel
	local f10_local1 = CoD.perController[controller].weaponCategory
	local f10_local2 = nil
	local f10_local3 = element.itemIndex
	local f10_local4 = CoD.PrestigeUtility.GetPermanentUnlockMode()
	if not f10_local3 then
		f10_local2 = element:getModel( controller, "itemIndex" )
		if f10_local2 then
			f10_local3 = Engine.GetModelValue( f10_local2 )
		end
	end
	if f10_local3 then
		local itemNumRef = tonumber( f10_local3 )
		if itemNumRef < 51 then
			-- we are checking if it's locked
			...
			if unlockData then
				...
				if str then
					...
					if not unlockArray[ itemNumRef ] then
						return true
					end

					return unlockArray[ itemNumRef ] == "0"
				end
			else
				return true
			end
		...
		if f10_local0 and f10_local1 and (LUI.startswith( f10_local1, "primaryattachment" ) or LUI.startswith( f10_local1, "secondaryattachment" )) then
			...
			if f10_local6 then
				return IsItemAttachmentLocked( controller, Engine.GetModelValue( f10_local6 ), f10_local3, "ICACILH", f10_local4 )
			end
		end
		...
		return Engine.IsItemLocked( controller, f10_local3, f10_local4 )
	else
		return false
	end
end
